...
class PackingPuzzleWizardWithMembers(PackingPuzzleWizard):

    def __init__(self, packing_Puzzle : SGP.Packing_Puzzle_data):
        self.puzzle = SGP.Packing_Puzzle(packing_Puzzle.size,packing_Puzzle.shapes)
        self.accumulators = dict()
        self.chugging = 0

    def solve_puzzle_one_pump(self):
        """returns false if working on it, returns true if done"""
        if PackingPuzzleWizard.get_block_unused(self.puzzle) != -1:
            self.chugging = self.chugging + 1
            if self.chugging == 100:
                self.chugging = 0
                print(".",end="")
            index : int
            broken_accumulator : tuple
            index = PackingPuzzleWizard.get_block_unused(self.puzzle)
            if not(index in self.accumulators):
                self.accumulators.update({index:0})
                
            broken_accumulator = PackingPuzzleWizard.break_down_accumulator(self.accumulators.get(index),self.puzzle)
            worked = PackingPuzzleWizard.jam_puzzle_block_in(self.puzzle,index,*broken_accumulator)
            if worked[0] == False:
                PackingPuzzleWizard.take_most_recent_block_out(self.puzzle)
                self.accumulators.update({index - 1 : self.accumulators.get(index - 1) + 1})
                self.accumulators.update({index: 0})
            else:
                self.accumulators[index] = PackingPuzzleWizard.reassemble_accumulator(worked[1],worked[2],worked[3],self.puzzle)
            return False
        else:
            return True


# test = SGP.Packing_Puzzle_data.txt_to_data("hPuzzle.txt")
# PPW = PackingPuzzleWizardWithMembers(test)
